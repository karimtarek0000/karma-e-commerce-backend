...
export const signIn = async (req, res, next) => {
  ...
  const accessToken = JWT.sign({ _id, email, userName }, process.env.ACCESS_TOKEN_SECRET, {
    expiresIn: "30s",
  });

  const refreshToken = JWT.sign({ _id, email, userName }, process.env.REFRESH_TOKE_SECRET, {
    expiresIn: "5d",
  });
